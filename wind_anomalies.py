...
def calculate_full_percentile(subws, value, threshold_value, threshold_percentile=99.5):
    subset_percentile = percentileofscore(subws, value, kind='rank')
    full_percentile = threshold_percentile + (subset_percentile * (100 - threshold_percentile) / 100)
    return full_percentile
...
allws = projected_ds["i10fg"].values.flatten()
allws = allws[allws>0]
percentile_99 = np.percentile(allws, 99)
full_percentile = calculate_full_percentile(allws[allws > percentile_99], 27, percentile_99)
subws = allws[allws > percentile_99]
