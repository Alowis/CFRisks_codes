...
list_of_word_vectors <- lapply(Hanze_flood_1980$Regions.affected..v2021., function(x) unlist(strsplit(x, ";")))
vectors_2021=unlist(list_of_word_vectors)
vectors_2021=unique(vectors_2021)
list_of_word_vectors2 <- lapply(Hanze_flood_1980$Regions.affected..v2010., function(x) unlist(strsplit(x, ";")))
...
dr_events<-dr_events[which(dr_events$cluster_length>=10),]
...
```{r Optimal flood detection window1, eval=FALSE}
initF=T
if (initF==T){
  HitF=c()
...
```{r}
#open csv form python
Hdbug<-read.csv(file=paste0(hydroDir,"/matchdebbug.csv"))

idshit<-which(is.na(match(Hanze_DFmatch$NID,Hdbug$eventID)))
merd<-Hanze_DFmatch[idshit,]
```
...
```
```{r}
allid=(Hanze_flood_1980$ï..ID)

mid=(unique(df_POTFlood_large$eventID))

length(which(is.na(match(allid,mid))))


#load event from python
df_FloodPy<-read.csv("K:/projects/CLIMEX/CFRisks/CFRisks_data/flood_matched.csv")

df_FloodPy$NuidT<-paste0(df_FloodPy$NUT,df_FloodPy$eventID,df_FloodPy$dtime2)
df_POTFlood_large$dtime1=round(df_POTFlood_large$dtime1)
df_POTFlood_large$dtime2=round(df_POTFlood_large$dtime2)
df_POTFlood_large$NuidT<-paste0(df_POTFlood_large$NUT,                             df_POTFlood_large$eventID,df_POTFlood_large$dtime2)

mpshit=full_join(df_FloodPy,df_POTFlood_large,by=c("NuidT"))
```
...
for (w in c(1:4)){
  ...
  plot1[[w]]<- ggplot(event_class2, aes(x = fct_rev(fct_infreq(event_name)),fill = nhcat))+ 
    geom_bar() + 
    coord_flip()+
    scale_fill_manual(
    ...
    scale_y_continuous(name="Number of events",breaks=seq(0,400, by=50),expand = expansion(mult = c(0, 0)))+
...
```{r}
list_test<-list_eventc2[[1]]
sum(list_test$predrought)
```
...
```{r Classification into conpound events archetypes}
